...
------------------XJ存储玩家相关数据---------------
function PlayerdataDAO.save_player_game_result(cmd, rid, resultinfo, condition) --function DatadbDao.insert(rid,  noticemsg)
	if cmd == nil or rid == nil or (resultinfo == nil and cmd ~= "delete") then
		filelog.sys_error("PlayerdataDAO.save_player_game_result invalid params")
		return
	end
	if resultinfo and type(resultinfo.players_name_coin_winlose) == "table" then --玩家每局游戏的结果{{角色table用字符串存的}}
		resultinfo.players_name_coin_winlose = json.encode(resultinfo.players_name_coin_winlose)
	end
	...
	local noticemsg = {
		...
		--rediscmd = "",
		mysqltable = "role_resultinfos",
		mysqldata = resultinfo, 
		mysqlcondition = nil, --是一个表格或者完整的字符串sql查询语句
		choosedb = 2,
	}
	...
	local f = dao[cmd]
	if f == nil then
		filelog.sys_error("PlayerdataDAO.save_player_game_result invalid cmd", cmd, resultinfo)
		return
	end
	f(rid, noticemsg)
end

function PlayerdataDAO.query_player_gameresult(rid, condition)
	local responsemsg
	local gameresult = {}
	if rid == nil then
		filelog.sys_error("PlayerdataDAO.query_player_gameresult invalid rid")
		return nil, nil
	end
	...
	responsemsg = dao.query(rid, requestmsg)
	if responsemsg == nil then
		filelog.sys_error("PlayerdataDAO.query_player_gameresult failed because cannot access datadbsvrd")
		return nil, nil		
	end

	if not responsemsg.issuccess then
		filelog.sys_error("PlayerdataDAO.query_player_gameresult failed because datadbsvrd exception")		
		return nil, nil
	end

	-- if (responsemsg.data == nil or tabletool.is_emptytable(responsemsg.data)) and responsemsg.isredisormysql then
	-- 	if playerdbinit == nil then
	-- 		playerdbinit = configdao.get_business_conf(100, 1000, "playerdbinit") ----业务需求来配置玩家数据初始值，比如玩家初始金币200
	-- 	local onegameresultinfo = tabletool.deepcopy(playerdbinit.gameresult)
	-- 	onegameresultinfo.rid = rid
	-- 	table.insert(gameresult,onegameresultinfo)
	-- 	return true, gameresult
	-- end
 
	if type(responsemsg.data) == "table" then   --把查询到的数据copy给宁外一个table
		for k,v in ipairs(responsemsg.data) do
			table.insert(gameresult, v) --数据插入到table
		end
	end

	return false, gameresult  --返回数据
end
------------------XJ数据存取--------------------------
