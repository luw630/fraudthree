...
function ReadyGame.process(session, source, fd, request)
	local responsemsg = {
	...
	if not msghelper:is_login_success() then
		filelog.sys_warning("ReadyGame.process invalid server state", server.state)
		responsemsg.errcode = EErrCode.ERR_INVALID_REQUEST
		responsemsg.errcodedes = "无效的请求！"
		msghelper:send_resmsgto_client(fd, "ReadyGameRes", responsemsg)		
		return
	end
	...
	responsemsg, seatinfo = msgproxy.sendrpc_reqmsgto_roomsvrd(nil, request.roomsvr_id, request.roomsvr_table_address, "readygame", request)
