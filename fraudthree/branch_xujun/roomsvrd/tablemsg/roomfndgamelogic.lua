...
function RoomGameLogic.continue(gameobj)
	...
	elseif tableobj.action_type == EActionType.ACTION_TYPE_CALL or tableobj.action_type == EActionType.ACTION_TYPE_RAISE  then --跟注
		local bets = 0
	...
	elseif tableobj.action_type == EActionType.ACTION_TYPE_COMPARE or tableobj.action_type == EActionType.ACTION_TYPE_FORCECOMPARE then --比牌
		local bets = 0
	...
	elseif tableobj.action_type == EActionType.ACTION_TYPE_RUSH then --血拼
		...
		--扣钱
		roomtablelogic.changeMoney(tableobj,seat,-bets,EReasonChangeCurrency.CHANGE_CURRENCY_SYSTEM_GAME) 
		--xj--
...
-----------------------------------------------跟注，比牌，加注，血拼----------------------------------
	...
	if isendgame == 1 then --判断是否结束游戏
		...
		--roomtablelogic.changeMoney(winseat,tableobj.all_bets )
...
function RoomGameLogic.onegameend(gameobj)
	...
	if tableobj.end_delete > 0 then    --桌子需要删除
		filelog.sys_error("在游戏中，进入了桌子需要删除状态")
		for _, seat in ipairs(tableobj.seats) do
			if seat.state > ESeatState.SEAT_STATE_NO_PLAYER then
...
function RoomGameLogic.onsitdowntable(gameobj, seat) --如果没有庄家，设置这个玩家为庄
	...
	-- 	roomtablelogic.startgame(gameobj.tableobj)
	-- end
...
--function RoomGameLogic.paymoney( gameobj, seat,is_endgame )
function RoomGameLogic.paymoney( gameobj, seat, is_endgame )
	local tableobj = gameobj.tableobj
	local noticemsg = 
	{
...
function RoomGameLogic.onegamestart_initseat(gameobj, seat)
	...
	seat.coin_base = seat.coin --记录当前的金币数

	--seat.room_type = 2
	--xj-
	local start_seat_info = {
		rid = seat.rid,                                      --刚坐下的人的rid
		name = seat.playerinfo.rolename,					 --角色名字
		--coin_enter = seat.coin_base,						 --暂时不考携带量的问题
	}
...
function RoomGameLogic.copy_gametable( gameobj,gametable )
	...
	gametable.room_type = tableobj.conf.room_type
    	...
    	filelog.sys_info("copy_gametable ",gametable.coin_realize)
...
function RoomGameLogic.recovery_gametable( gameobj,gametable )
	...
	tableobj.conf.retain_time = gametable.retain_time 
	 ...
	 tableobj.coin_realize = {}
    	for k,v in pairs(gametable.coin_realize) do
    		tableobj.coin_realize[k] = v
    	end
	...
	--tableobj.conf.coin_realize = tabletool.deepcopy(gametable.coin_realize)
    --tableobj.conf.start_game_player_info = tabletool.deepcopy(gametable.start_game_player_info)
