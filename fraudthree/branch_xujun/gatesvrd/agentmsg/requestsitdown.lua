...
function  RequestSitdown.process(session, source, fd, request)
	local responsemsg = {
	...
	if not msghelper:is_login_success() then
		filelog.sys_warning("RequestSitdown.process invalid server state", server.state)
		responsemsg.errcode = EErrCode.ERR_INVALID_REQUEST
		responsemsg.errcodedes = "无效的请求！"
		msghelper:send_resmsgto_client(fd, "RequestSitDownRes", responsemsg)		
		return
	end
	...
	if server.roomsvr_id == "" or server.roomsvr_id ~= request.roomsvr_id then
		...
		responsemsg.errcodedes = "无效的请求参数--roomsvr_id！"
		msghelper:send_resmsgto_client(fd, "RequestSitDownRes", responsemsg)		
		return		
	end
	...
	if server.roomsvr_table_id <= 0 or server.roomsvr_table_id ~= request.id then
		responsemsg.errcode = EErrCode.ERR_INVALID_PARAMS
		responsemsg.errcodedes = "无效的请求参数！--roomsvr_table_id"
		msghelper:send_resmsgto_client(fd, "RequestSitDownRes", responsemsg)		
		return		
	end
	...
	request.sitdown_rolename =  server.info.rolename
	...
	if not msghelper:is_login_success() then
		filelog.sys_info("not msghelper:is_login_success ",responsemsg)
		return
	end
