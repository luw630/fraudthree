...
function  EnterGame.process(session, source, fd, request)
	...
	---填充
	msghelper:generate_mail(request.rid,newplayermail,ESendMailReasonType.COMMON_TYPE_TESTING)
	---填充一封有附件的邮件，测试
	local attachmail = tabletool.deepcopy(mailconf.testplayermail)
	msghelper:generate_mail(request.rid,attachmail,ESendMailReasonType.COMMON_TYPE_TESTING)
	...
	gamelog.write_player_loginlog(status, server.uid, server.rid, server.regfrom, server.platform, server.channel, 
		server.authtype, server.version,timetool.get_time(),server.ip)
